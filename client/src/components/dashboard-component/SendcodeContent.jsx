...
const SendCode = () => {
  ...
  const SendCodeSchema = Yup.object().shape({
    codeLink: Yup.string().test('is-referral','*Invalid Link Referral',(value) =>{
    ...
    email: Yup.string()
       ...
       .test("is-unique", "*Email address is already sent for referral", (value) => {return !emailList.includes(value);
      })   
    });
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const response = await axios.patch(`http://localhost:2000/users/${user.id}`, {
    ...
    toast({
     ...
     if(currentPoints === 100){
      ...
      alert(`Congratulation now you have 10% discount for buying a ticket event..!!`)
     } else if (currentPoints === 1000)
     alert(`Congratulation now you have 50% discount for buying a ticket event..!!`)    
